...
if (!is_null($events['events'])) {
	...
	foreach ($events['events'] as $event) {
		...
		if ($event['type'] == 'message' && $event['message']['type'] == 'text') {
			...
			$register_text = " กดที่ลิงค์นี้เพื่อลงเบียนผู้ใช้งาน http://ellis.surat.psu.ac.th/Line-Messaging-API-V2/staff_register.php?line_user_id=".$Line_UserId."&apikey=".$apikey;
